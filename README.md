# 🤖 Daft Punk Vocal DSP Engine

Turn **text into Daft-Punk-style robot vocals** in the browser, with a wall of
sliders to play with. Type a phrase, pick a preset, and the engine speaks it,
auto-tunes it, vocodes it through a polyphonic synth, and runs it through a
talkbox + saturation + phaser + sidechain + stereo chain.
...
vocal-processing libraries** for any of the core algorithms.

```
 text ─► [espeak TTS] ─► [PSOLA hard auto-tune] ─┐
                                                 ▼
 chord ─► [polyphonic synth] ───────────► [32-band channel vocoder] ─► [formant / talkbox]
                                                 │
                                                 ▼
...
Then open **http://localhost:8000**, type some text, hit **SYNTHESIZE**, and
tweak the sliders. A typical phrase renders in well under a second.
...
|---|---|---|
| `/` | GET | the slider UI |
| `/api/meta` | GET | default params, presets, and option lists |
| `/api/synthesize` | POST | JSON params → `{audio_b64 (16-bit WAV), meta}` |
| `/api/synthesize.wav` | POST | same params → WAV download (`?float32=1` for 32-bit float) |
